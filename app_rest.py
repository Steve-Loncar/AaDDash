...
# -----------------------------
# Heatmap - all (appended helpers + tab UI)
# -----------------------------
    ...
    for _, row in df_metric.iterrows():
        ...
        for hc in hierarchy_cols:
            ...
            if pd_is_valid := (val is not None and str(val).strip() != "" and str(val).lower() not in ["nan", "none"]):
                ...
                # when a level is missing we break (no deeper ancestors)
                break
...
def build_heatmap_matrix_from_paths(df, metric_name="EBITDA margin 2025", value_col="Value"):
    ...
    if not full_paths:
        return np.empty((0, len(hcols))), [], hcols
    ...
    for path in full_paths:
        ...
        for level_idx in range(len(hcols)):
            if level_idx < len(parts):
                ...
                row_vals.append(metric_map.get(ancestor_path, np.nan))
            else:
...
def render_heatmap_figure(matrix, row_labels, col_labels, metric_name="Metric", colorscale="RdYlGn"):
    ...
    if finite.any():
        zmin = float(np.nanmin(np.where(finite, z, np.nan)))
        zmax = float(np.nanmax(np.where(finite, z, np.nan)))
    else:
        zmin, zmax = 0.0, 1.0
...
def heatmap_all_tab_ui(df, default_metric="EBITDA margin 2025", value_col="Value"):
    ...
    metrics_options = [default_metric]
    try:
        if df is not None and 'Fiscal Metric Flattened' in df.columns:
            uniq = pd.Series(df['Fiscal Metric Flattened'].dropna().astype(str).unique()).tolist()
            if uniq:
                metrics_options = uniq
    except Exception:
        pass

    metric = st.selectbox("Determining factor (metric)", options=metrics_options, index=0, help="Choose metric used to color the heatmap. You can add more metrics to the selectbox if desired.")
    ...
    if st.download_button("Download matrix CSV", data=pd.DataFrame(matrix, index=row_labels, columns=col_labels).to_csv(index=True), file_name="heatmap_all_matrix.csv"):
        st.success("CSV prepared")


# Wire the tab UI into the app
with tab_heatmap_all:
    df_active = get_active_df()
    heatmap_all_tab_ui(df_active)
