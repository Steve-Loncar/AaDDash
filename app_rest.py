...
def show_plotly(fig: go.Figure, height: Optional[int] = None):
    """Render a Plotly figure consistently without deprecated kwargs."""
    cfg = {"displayModeBar": False}
    # If height not explicitly provided, try to pick up figure layout height
    if height is None:
        try:
            h = fig.layout.height
            if h is not None:
            ...
            height = None
    if height:
        st.plotly_chart(fig, use_container_width=True, config=cfg, height=height)
    else:
